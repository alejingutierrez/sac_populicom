...
} from "../packages/db/src/enrichments.ts";

const scriptDir = path.dirname(fileURLToPath(import.meta.url));
const rootDir = path.resolve(scriptDir, "..");
const docsDir = path.join(rootDir, "docs", "enrichments");

const apiMarkdown = `# API de Enrichments

- Total de derivadas expuestas por catálogo: \`${enrichmentDefinitions.length}\`
...
fs.writeFileSync(
  path.join(docsDir, "catalog.md"),
  `${renderEnrichmentCatalogMarkdown()}\n`
);
fs.writeFileSync(
  path.join(docsDir, "formulas.md"),
  `${renderEnrichmentFormulasMarkdown()}\n`
);
fs.writeFileSync(path.join(docsDir, "api.md"), `${apiMarkdown}\n`);

console.log(`Generated docs in ${docsDir}`);
