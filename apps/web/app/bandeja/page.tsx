...
const BandejaPage = async ({ searchParams }: BandejaPageProps) => {
  ...
  const selectedAgencyId =
    typeof params.agencyId === "string"
      ? params.agencyId
      : session.activeAgencyId;
  const selectedMentionId =
    typeof params.mentionId === "string" ? params.mentionId : undefined;
  const [cases, mentions, agencies, users] = await Promise.all([
    repository.listCases(session, selectedAgencyId),
    repository.listMentions(session, { agencyId: selectedAgencyId }),
    ...
    >
      <section className="panel-grid">
        ...
        <section className="panel">
          <SectionHeading
            title={`Casos activos en ${selectedAgencyId}`}
            description="Actualiza estado, prioridad y responsable."
          />
          <div className="stack">
            {cases.map((record) => (
