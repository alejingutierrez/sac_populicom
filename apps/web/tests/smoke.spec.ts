...
test("dashboard loads and navigates through core modules", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Exploración analítica" })
  ).toBeVisible();
  await expect(
    page.getByText("Radar exploratorio del monitoreo")
  ).toBeVisible();
  await expect(page.getByText("Volumen por plataforma")).toBeVisible();
...
test("can create and update a case", async ({ page }) => {
  ...
  await page
    .getByLabel("Resumen")
    .fill("Caso de prueba para validar el flujo inicial.");
  await page.getByRole("button", { name: "Crear caso" }).click();

  await expect(
    page.getByText("Validación operativa desde smoke")
  ).toBeVisible();
  await page
    .locator(".case-inline-editor")
    .first()
    .getByRole("combobox")
    .first()
