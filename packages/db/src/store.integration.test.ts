...
describe("memory repository integration", () => {
  it("deduplicates mentions during upsert", async () => {
    const repository = createMemoryRepository(createSeedData());
    const session = getDemoSession();
    const existingMention = (await repository.listMentions(session))[0];
    if (!existingMention) {
      throw new Error("Expected seeded mention");
    }

    const result = await repository.upsertMentions([existingMention], {
  ...
  it("creates alerts for critical mentions", async () => {
    const repository = createMemoryRepository(createSeedData());
    ...
    const result = await repository.upsertMentions([mention], {
